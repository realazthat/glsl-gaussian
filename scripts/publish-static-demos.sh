...
if [ "$TRAVIS_REPO_SLUG" == "realazthat/glsl-gaussian" ] && [ "$TRAVIS_PULL_REQUEST" == "false" ] && [[ $PUBLISH_BRANCHES =~ "$TRAVIS_BRANCH" ]]; then
  ...
  rm -rf gh-pages
  git clone --quiet --branch=gh-pages https://${GH_TOKEN}@github.com/realazthat/glsl-gaussian gh-pages > /dev/null
